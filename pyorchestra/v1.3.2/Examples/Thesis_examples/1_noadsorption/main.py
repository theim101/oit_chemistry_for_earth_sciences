...
height = 1 #[cell unit]
nIN = 2 #number of cell boundaries. 2 for a 0D system (top and bottom)

#path of input files
ORCHESTRA_chemistry = "chemistry.inp" #ORCHESTRA input file
reaction_file = "reactions.csv" #stoichiometry input file

##############################################################
#    CHANGE HERE WASTE COMPOSITION !!
##############################################################
comp_waste = np.array([1, 1.72, 0.5, 0.03125]) #[C, H, O, N]
comp_waste = comp_waste/comp_waste[0] #normalized to C (first index)

newline = '@composition(waste, ' + str(comp_waste[0]) + ', CO3-2, ' + str(comp_waste[1] - 4*comp_waste[3]) + ', H, ' + str(comp_waste[2] - 3*comp_waste[0]) + ', O)\n' #line to change in ORCHESTRA input file
comp_waste_new = np.array([comp_waste[0], comp_waste[1]-4*comp_waste[3], comp_waste[2]-3*comp_waste[0], comp_waste[3]]) #get composition for primary entities 

#===================================================================
#change waste composition in chemistry file:
with open(ORCHESTRA_chemistry) as f:
    data = f.readlines()
    line_number = 0
    for line in data:
        line_number +=1
...
with open(ORCHESTRA_chemistry, 'w') as file:
    file.writelines( data )

#==================================================================
#calculate amount of moles
molar_mass_waste = 12*comp_waste[0] + 1.008*comp_waste[1] + 16*comp_waste[2] + 14*comp_waste[3] #g/mol for SOM
mol_waste = total_organic_waste *1000 /molar_mass_waste #mol OM
...
# =============================================================================
# initialise ORCHESTRA 
# =============================================================================
p, IN = Forch.ORCHESTRA_ini(ORCHESTRA_chemistry, reaction_file, mDim)

# =============================================================================
# set initial conditions (totals)
# =============================================================================
...
#orchestra totals
IN[np.where(IN == 'CO3-2.tot')]  = 0.27+ comp_waste_new[0]*total_organic_waste*1000/molar_mass_waste #total for liquid + total in SOM
IN[np.where(IN == 'H.tot')]  = 0.27 + comp_waste_new[1]*total_organic_waste*1000/molar_mass_waste #total for liquid + total in SOM
IN[np.where(IN == 'NH4+.tot')]  = 0.0
IN[np.where(IN == 'NO3-.tot')] = 1.0e-9 #
IN[np.where(IN == 'Ca+2.tot')]  = 0.47 #2
IN[np.where(IN == 'N2.tot')]  = 1e-9 #2
IN[np.where(IN == 'waste.tot')]  =  total_organic_waste * 1000/molar_mass_waste #total SOM 
...
if plot == 'yes':
    ...
    pH, O_act, O2_act, ammonium_aq, nitrate_aq, activities_aq = [], [], [] , [], [], []
    f_main_inhs, f_pH_inhs, f_NH4_inhs, f_X_inhs, f_NH4_tox_inhs = [], [], [], [], []
    f_O2_tox_inhs, f_O2_inhs, f_NO3_inhs = [], [], []

    #loop over every timestep to get remaining parameters
    for sBio in int_result.y[mDim.nN*0:mDim.nN*16].T:
        #get input
        INvars = np.array([sBio])
        INvars = np.where(INvars < 0, 1e-15, sBio)

        #get output
        OUT = p.set_and_calculate(INvars)

        #store output values in lists
        pH.append(OUT[0][16])
        ...
        #calculate inhibitions using output
        ...
        #store inhibition values in lists
    ...
    #convert inhibitions to np arrays
    ...
    fig.suptitle('Model results for a (partly) open system with no adsorption', fontsize=20)
    
    #set background gradient (oxygen level)
    cmap = plt.cm.cool 
    ...
    #add figures/legends/labels/etc.
    ...
    ax[0,1].set_title('$\\bf{NH_4^+}$ and $\\bf{NO_3^-}$')
    ...
    ax[2,1].plot(tOut/365.25, CO2.T, color='k')
    ax[2,1].set_title('$\\bf{CO_2}$')
    ax[2,1].set_ylabel('[mol]')
    ax[3,1].plot(tOut/365.25, CH4.T, color='k')
    ...
    ax[2,0].set_ylim(0.0,max(methanogens.T)+0.1*max(methanogens.T))
    ...
    ax[2,1].set_ylim(0, max(CO2.T)+0.05*np.mean(CO2))
    ax[3,1].set_ylim(0, max(CH4.T)+0.05*np.mean(CH4))    
    ...
    plt.tight_layout()
    plt.savefig('components_aeration.pdf') #save as pdf
    plt.show()

    #second plot for the inhibitions!
    ...
    plt.tight_layout()
    plt.savefig('inhibitions_aeration.pdf')
    plt.show()

    fig = plt.figure(constrained_layout=True, figsize=(7,5))
    gs = GridSpec(2, 6, figure=fig)
    fig.suptitle('Closed system with no adsorption (0-15 years)', fontsize=20)
    ax1 = fig.add_subplot(gs[0, :3])
    ax2 = fig.add_subplot(gs[0, 3:])
    ax3 = fig.add_subplot(gs[1, :3])
    ax4 = fig.add_subplot(gs[1,3:])

    ax1.plot(tOut/365.25, Waste1.T, color='k',zorder=2)
    ax1.set_title('$\\bf{SOM}$')
    ax1.set_ylabel('[mol]')
    ax2.plot(tOut/365.25, acetate.T, color='k')
    ax2.set_title('$\\bf{acetate}$')
    ax2.set_ylabel('[mol/L]')
    ax3.plot(tOut/365.25, biomass1.T, color='k', label='$X_{meth}$')
    ax3.plot(tOut/365.25, biomass2.T, color='r', label = '$X_{ox}$')
    ax3.plot(tOut/365.25, biomass3.T, color='k', ls='--', label='$X_{nitr}$')
    ax3.legend(loc='upper right', fontsize='small')
    ax3.set_title('$\\bf{biomass}$')
    ax3.set_ylabel('[mol/L]')
    ax4.plot(tOut/365.25, O2_act, color='k')
    ax4.set_title('$\\bf{O_2^{aq}}$')
    ax4.set_ylabel('[mol/L]')
    ax1.set_xlabel('time (year)')
    ax2.set_xlabel('time (year)')
    ax3.set_xlabel('time (year)')
    ax4.set_xlabel('time (year)')

    ax1.set_ylim(2.58,3.120)
    ax2.set_ylim(0.0, 0.07)
    ax3.set_ylim(0,0.06)
    ax4.set_ylim(0,0.0004)
    ax1.set_xlim(0,15)
    ax2.set_xlim(0,15)
    ax3.set_xlim(0,15)
    ax4.set_xlim(0,15)
    ...
    fig.suptitle('$CO_2$ and $CH_4$ production', fontsize=20)
    ax1 = fig.add_subplot(gs[0, :1])

    ax1.plot(tOut/365.25, CO2.T, color='k', zorder=2, label='$CO_2$')
    ax1.set_ylabel('[mol]')
    ax1.plot(tOut/365.25, CH4.T, color='r', zorder=2, label='$CH_4$')
    ...
    for ii in range(len(tOut)-1):
        print(f'plotting... please wait... {ii/(len(tOut)-1) * 100} %')
        ax1.fill_between([tOut[ii]/365.25, tOut[ii+1]/365.25], -0.1,10, color=cmap(norm(O2_act[ii])),zorder=1, lw=0)
    plt.savefig('Case_1_gas.pdf')
