...
def parameters(nIN, height, reaction_file):
    ...
    def system_lid(t):
        cycle = 0 + 1*math.sin((math.pi*2/(365.25*5) * t/2)) +0.97
        switch = 1*(cycle<0) + 0
        return switch
...
def read_input_file(reaction_file, comp_waste):
    ...
    #--------------------------
    df_parameters = df[['mu_max', 'K_pH', 'pH_opt1', 'pH_opt2', 'kT', 'T_opt', 'K_tox_O2', 'K_lim_O2', 'K_tox_NH4', 'K_lim_NH4' , 'K_lim_NO3','limiting_substrate', 'K_X', 'K_lim_main', 'monitoring_species', 'DG_dis']] #get index with all parameters
