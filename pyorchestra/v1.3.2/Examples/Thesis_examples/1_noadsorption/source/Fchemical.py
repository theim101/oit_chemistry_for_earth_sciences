...
def Flambda_cat(stoich, activities, sBio, T):
    ...
    #------------------------------------------------------
    # calculate DG_diss = G_diss * mol biomass
    #------------------------------------------------------    
...
def Finhibitions(sBio, activities, stoich, pH, T):
...
def Fchemrates(t, sBio, sGas, pH, O2gas_activity, activities, sPar, bPar, stoich):
    """calculate inhibition functions to determine rate for chemical processes"""
    #some constants...
    ...
    f_tot = Finhibitions(sBio, activities, stoich, pH, T)
    ...
    R[9] = switch* np.array(stoich.params['mu_max'])[9] * (0.2 - O2gas_activity)/0.2 #SPECIAL CASE WHERE THE OXYGEN IS ADDED (is 6th reaction in input file)!!!
    ...
    M_normalized = M_normalized*R #for every individual reaction 
    M_normalized = M_normalized.sum(axis=0) #sum of reactions
    
    #------------------------------------------------------
    # finalize rate vector
    # HERE IS A PIECE OF HARDCODING, NUMBER OF MINITORING SUBSTRATE IS UNKOWN IN M_NORM. 
    # NEED TO KNOW AMOUNT OF TOTAL ELEMENTS (E.G. C.tot/H.tot/N.tot) TO SPECIFY LENGTH CORRECTLY
    #------------------------------------------------------
    R_bio = np.zeros_like(sBio)
    R_bio[3:-3] = np.array([M_normalized[:10]]).T
    
    R_gas = np.zeros_like(sGas)
    R_gas[:] = np.array([M_normalized[10:12]]).T
    return R_bio, R_gas
...
def FCR(t, sBio, sGas, p, sPar, bPar, stoich):
    ...
    #------------------------------------------------------
    equilibrium = p.set_and_calculate(sBio.T)
    #------------------------------------------------------
    # get values of interest (HRDCODED)
    #------------------------------------------------------
    pH = equilibrium[0][16]
    ...
    #------------------------------------------------------
    R_bio, R_gas = Fchemrates(t, sBio, sGas, pH, O2gas_activity, activities, sPar, bPar, stoich) 
