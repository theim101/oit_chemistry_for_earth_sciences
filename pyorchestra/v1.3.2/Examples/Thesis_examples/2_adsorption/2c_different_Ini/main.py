...
fractions = np.array([fraction1, fraction2, fraction3, fraction4])

results = []
for ii, organic_waste_fraction in enumerate(fractions):
...
fig.suptitle('Effects of initial organic fraction for a system with $NH_4^+$ adsorption', fontsize=20)
linestl = ['-', '-','-','-','-','-']
alpha = [1, 1, 1, 1,1,1]
colors= ['k','blue','red','green','c','k']
labels = ['fraction = 0.1','fraction = 0.2', 'fraction = 0.3', 'fraction = 0.4']
...
plt.tight_layout()
plt.savefig('components_case2c.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig('biomass_case2c.pdf')
plt.show()
