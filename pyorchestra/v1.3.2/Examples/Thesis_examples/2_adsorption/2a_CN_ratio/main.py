...
composition3 = np.array([1, 1.72, 0.5, 0.03125]) #1:32
composition1 = np.array([1, 2.395, 0.5, 0.2]) #1:5
composition2 = np.array([1, 1.88071428571429, 0.5, 0.071428571428572]) #1:14
composition4 = np.array([1, 1.675, 0.5, 0.02]) #1:50
 
composition = np.array([composition1, composition2, composition3, composition4])

results = []
for comp_waste in composition:
    ...
    newline2 = '@logKreaction(NH4_ads, -1' + ', "0.8, 1.0" , NH4+)\n'
    #===================================================================
...
fig.suptitle('Effects of C/N ratio for a system with $NH_4^+$ adsorption', fontsize=20)
linestl = ['-', '-','-','-','-','-']
alpha = [1, 1, 1, 1,1,1]
colors= ['k','blue','red','green','c','k']
labels = ['C:N = 5:1 ','C:N = 14:1', 'C:N = 32:1', 'C:N = 50:1']
for ii, int_result in enumerate(results):
    comp_waste = composition[ii]
    comp_waste = comp_waste/comp_waste[0] #normalized to C (first index)
    newline = '@composition(waste, ' + str(comp_waste[0]) + ', CO3-2, ' + str(comp_waste[1] - 4*comp_waste[3]) + ', H, ' + str(comp_waste[2] - 3*comp_waste[0]) + ', O)\n'
    comp_waste_new = np.array([comp_waste[0], comp_waste[1]-4*comp_waste[3], comp_waste[2]-3*comp_waste[0], comp_waste[3]])
    newline2 = '@logKreaction(NH4_ads, -1' + ', "0.8, 1.0" , NH4+)\n'
...
plt.tight_layout()
plt.savefig('components_case2a.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig('biomass_case2a.pdf')
plt.show()
