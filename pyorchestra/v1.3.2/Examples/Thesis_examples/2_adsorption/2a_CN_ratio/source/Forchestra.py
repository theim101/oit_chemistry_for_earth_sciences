...
def ORCHESTRA_ini(chemistry_file, reaction_file, mDim):
    ...
    IN = np.array(df_orch.filter(like='.tot').columns) #input, all column names with .tot =INVAR
    IN = np.append(IN, ['porosity', 'saturation', 'wastefraction']) #add porosity, saturation and wastefraction at the end of the list

    OUT = np.array(df_orch.loc[:, df.loc['is_output'] == 'yes'].columns).astype('str') #get columns for output =OUTVAR
